...
const FireAnimation: React.FC<{
...
}> = React.memo(({ id, x, y, type, onComplete, style }) => {
  useEffect(() => {
  ...
  return (
    <img
      src={type === 'my' ? "/myfire.svg" : "/enimefire.svg"}
      alt="fire"
      className={`fire-effect ${type === 'my' ? 'my-fire' : 'enemy-fire'}`}
      style={{
        position: 'absolute',
        left: `${x}px`,
        top: `${y}px`,
...
const FightPage: React.FC = () => {
  ...
  const [enemyBullets, setEnemyBullets] = useState(14);
  ...
  const handleShotFireAnimationComplete = useCallback((id: string) => {
    setShotFireEffects(prev => prev.filter(fire => fire.id !== id));
  }, []);
    ...
    const battleInterval = setInterval(() => {
      ...
      if (enemyBullets > 0) {
        ...
        if (activeMySoldiers.length > 0 && activeEnemySoldiers.length > 0) {
          const targetIndex = Math.floor(Math.random() * activeMySoldiers.length);
          const attackerIndex = Math.floor(Math.random() * activeEnemySoldiers.length);

          const targetId = activeMySoldiers[targetIndex].id;
          const attackerId = activeEnemySoldiers[attackerIndex].id;

          const attackerElem = document.querySelector(`[data-soldier-id="${attackerId}"] .enimy`);
          const targetElem = document.querySelector(`[data-soldier-id="${targetId}"] .myarmy`);
          ...
          if (attackerElem && targetElem) {
            ...
            setTimeout(() => {
              setMyArmy(prev => prev.map(s =>
                s.id === targetId ? { ...s, isHit: true } : s
              ));

              // Show hit effect
              setImpactFireEffects(prev => [...prev, {
              ...
              setTimeout(() => {
                setMyArmy(prev => prev.map(s =>
                  s.id === targetId ? { ...s, destroyed: true, isHit: false } : s
                ));
      ...
      if (myBullets > 0) {
        ...
        if (activeEnemySoldiers.length > 0 && activeMySoldiers.length > 0) {
          const targetIndex = Math.floor(Math.random() * activeEnemySoldiers.length);
          const attackerIndex = Math.floor(Math.random() * activeMySoldiers.length);

          const targetId = activeEnemySoldiers[targetIndex].id;
          const attackerId = activeMySoldiers[attackerIndex].id;

          const attackerElem = document.querySelector(`[data-soldier-id="${attackerId}"] .myarmy`);
          const targetElem = document.querySelector(`[data-soldier-id="${targetId}"] .enimy`);
  ...
  useEffect(() => {
    ...
    if (myActive === 0 || enemyActive === 0 || (myBullets === 0 && enemyBullets === 0)) {
      setBattleOver(true);
      ...
      if (myActive > enemyActive) {
        ...
        const myPoints = myArmy.reduce((sum, s) => sum + (s.destroyed ? 0 : s.points), 0);
        const enemyPoints = enemyArmy.reduce((sum, s) => sum + (s.destroyed ? 0 : s.points), 0);
  ...
  const resetBattle = () => {
    ...
    setImpactFireEffects([]);
    setShotFireEffects([]);
    setMyArmy(prev => prev.map(s => ({ ...s, destroyed: false, isHit: false })));
    setEnemyArmy(prev => prev.map(s => ({ ...s, destroyed: false, isHit: false })));
  };

  const generateTriangleFormation = useCallback((soldiers: Soldier[]): number[][] => {
  ...
  const renderFormation = useCallback((soldiers: Soldier[], type: "my" | "enemy") => {
    const formation = generateTriangleFormation(soldiers);
    const mirroredFormation = type === "enemy" ? mirrorFormation(formation) : formation;
            ...
            {row.map((cell, colIndex) => {
              if (cell === 1 && activeSoldiersCopy.length > 0) {
                ...
                const isFiring = shotFireEffects.some(fire => {
                  const soldierElem = document.querySelector(`[data-soldier-id="${soldier.id}"]`);
                ...
                return (
                  ...
                  >
                    <div className={`${type === "my" ? "army" : "enemy"} ${soldier.destroyed ? "destroyed-soldier" : ""}`}>
                      <img
                        src={type === "my" ? "/fight-page-my-army.svg" : "/enime.svg"}
                        alt={type === "my" ? "myarmy" : "enemy"}
                        className={`${type === "my" ? "myarmy" : "enimy"} ${soldier.isHit ? "hit-effect" : ""}`}
                      ...
                      {isFiring && (
                        <div className="bullet-animation-container">
                          <img
                            src={type === "my" ? "/myfire.svg" : "/enimefire.svg"}
                            alt="bullet"
                            className={`bullet ${type === "my" ? "my-bullet" : "enemy-bullet"}`}
                          />
                        </div>
                      )}
                      ...
                      {isBeingHit && (
                        <div className="hit-animation-container">
                          <img
                            src={type === "my" ? "/enimefire.svg" : "/myfire.svg"}
                            alt="hit"
  ...
  }, [generateTriangleFormation, mirrorFormation, shotFireEffects, impactFireEffects, handleImpactFireAnimationComplete]);
  const renderProjectiles = useCallback(() => {
    return projectiles.map(proj => {
      ...
      return (
        <div
          key={proj.id}
          className={`projectile ${proj.from}-projectile`}
          style={{
            position: 'fixed',
            left: `${currentX}px`,
            top: `${currentY}px`,
  ...
  const totalMyPoints = myArmy.reduce((sum, soldier) => sum + (soldier.destroyed ? 0 : soldier.points), 0);
  const totalEnemyPoints = enemyArmy.reduce((sum, soldier) => sum + (soldier.destroyed ? 0 : soldier.points), 0);

  return (
    <div className="game-content blurred">
      <div className="provided-bullets btn btn-outline-dark">My Bullets: {myBullets}</div>
      <div className="enemy-bullets btn btn-outline-danger">Enemy Bullets: {enemyBullets}</div>
      ...
      {battleOver && (
        <div className="battle-result">
          <h2>
            {winner === "my" ? "You Won!" :
              winner === "enemy" ? "Enemy Won!" : "It's a Draw!"}
          </h2>
